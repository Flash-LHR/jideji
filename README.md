...
- 自定义标签的名称、Emoji 或相册图片、颜色和顺序；图片可裁剪，可停用及恢复；
- 完整导出 ZIP：含四个 Sheet 的 Excel、账单原图和标签图片；
...
账单数据库、照片和标签图片位于应用私有目录，Android 云备份和设备迁移备份均已禁用。完整导出是未加密 ZIP。除 GitHub Release 更新接口和安装包外，App 不主动访问其他网络服务；地点文字由安卓系统的反向地理编码能力提供。
...
Debug 包与正式发布包的签名不同，不能直接覆盖安装。不要在 Debug 包里长期记录真实数据；卸载 Debug 包会同时清除其中的账单和图片。
...
每次发布前先更新根目录的 `release-notes.md`，每行写一条 `- 更新内容`，最多四条。工作流会把同一份内容用于 GitHub Release 和应用内更新说明，避免重复维护。

工作流会自动测试、生成同签名 APK，并按 `jideji-v版本号.apk` 命名后与 `update.json` 一起上传到对应的 GitHub Release。App 通过稳定的 Release 下载地址发现最新版本，不占用 GitHub REST API 限额。发布包会自动写入当前仓库的 owner/repo；后续发布必须使用更高版本标签，并继续使用同一把签名密钥。App 下载完成前不限制使用，安装包完整且通过包名、版本号和签名校验后才会显示不可跳过的更新页与本次更新内容。用户点击“更新并重启”后，Android 12 及以上会优先自动覆盖更新，并在安装成功后由系统回调重新打开 App；如果系统要求用户确认则自动转入系统确认页。Android 8 至 11 始终使用系统确认页。
