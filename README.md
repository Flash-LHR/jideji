...
工作流会自动测试、生成同签名 APK，并按 `jideji-v版本号.apk` 命名后与 `update.json` 一起上传到对应的 GitHub Release。App 通过稳定的 Release 下载地址发现最新版本，不占用 GitHub REST API 限额。发布包会自动写入当前仓库的 owner/repo；后续发布必须使用更高版本标签，并继续使用同一把签名密钥。App 下载完成前不限制使用，安装包完整且通过包名、版本号和签名校验后才会显示不可跳过的更新页与本次更新内容。用户点击“更新并重启”后，Android 12 及以上会优先自动覆盖更新；如果系统要求用户确认则自动转入系统确认页。Android 8 至 11 始终使用系统确认页。
