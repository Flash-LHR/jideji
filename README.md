...
工作流会自动测试、生成同签名 APK，并按 `jideji-v版本号.apk` 命名后与 `update.json` 一起上传到对应的 GitHub Release。App 通过稳定的 Release 下载地址发现最新版本，不占用 GitHub REST API 限额。发布包会自动写入当前仓库的 owner/repo；后续发布必须使用更高版本标签，并继续使用同一把签名密钥。App 下载完成前不限制使用，安装包完整且通过包名、版本号和签名校验后才会强制更新；最终安装确认仍由安卓系统完成。
